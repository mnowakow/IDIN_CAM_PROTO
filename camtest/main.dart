...
class _CameraExampleHomeState extends State<CameraExampleHome>
  ...
  // Image stream for pixel data analysis
  ...
  void dispose() {
    ...
    _stopImageStream(); // Image Stream stoppen
  ...
  // Image Stream Methods for Pixel Data Access
  Future<void> _startImageStream() async {
    ...
    try {
      ...
      showInSnackBar('Image stream started - analyzing pixels');
    } catch (e) {
      showInSnackBar('Error starting image stream: $e');
    }
  }

  void _processImage(CameraImage image) {
    // Hier können Sie die Pixeldaten verarbeiten
    // Format-Informationen
    print('Image format: ${image.format.group}');
    print('Image size: ${image.width}x${image.height}');

    // Zugriff auf Pixel-Planes (meist YUV format)
    for (int i = 0; i < image.planes.length; i++) {
      final plane = image.planes[i];
      print(
        'Plane $i: ${plane.bytes.length} bytes, bytesPerRow: ${plane.bytesPerRow}',
      );

      // Beispiel: Erste 10 Bytes ausgeben
      if (plane.bytes.length >= 10) {
        final firstBytes = plane.bytes.sublist(0, 10);
        print('First 10 bytes of plane $i: $firstBytes');
      }
    }

    // Hier könnten Sie Computer Vision, ML, Objekterkennung etc. implementieren
    // Beispiel: Helligkeit berechnen (vereinfacht)
    if (image.planes.isNotEmpty) {
      final yPlane = image.planes[0]; // Y-Plane (Luminanz)
      int sum = 0;
      final sampleSize =
          (yPlane.bytes.length / 100).round(); // Sample 1% der Pixel

      for (int i = 0; i < yPlane.bytes.length; i += sampleSize) {
        sum += yPlane.bytes[i];
      }

      final avgBrightness = sum / (yPlane.bytes.length / sampleSize);
  ...
  Future<void> _stopImageStream() async {
    if (controller != null && controller!.value.isStreamingImages) {
      ...
      showInSnackBar('Image stream stopped');
    }
  }
}
